class VeterinaryClinic {
    ...
    toString() {
        ...
        let finalPets = this.clients.map(c => c.pets.filter(pet => pet.length > 0));
        let result = [
            `${this.clinicName} is ${Math.floor((finalPets.length / this.capacity) * 100)}% busy today!`,
            `Total profit: ${this.totalProfit.toFixed(2)}$`,
            `${this.clients.sort((a, b) => a.name.localeCompare(b.name))
                .map(c => `${c.name} with: \n--${c.pets.sort((a, b) => a.name.localeCompare(b.name))
                    .map(p => `${p.name} - a ${p.kind.toLowerCase()} that needs: ${p.procedures.map(pr => `${pr}`)}`)
                    .join('\n')}`).join('\n')}`
        ]
